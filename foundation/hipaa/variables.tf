...
# ---------------------------------------------------------------------------
# HIPAA-specific variables
# ---------------------------------------------------------------------------

variable "compliance_regime" {
  description = "The Assured Workloads compliance regime to apply to all environments."
  type        = string
  default     = "HIPAA"

  validation {
    condition     = contains(["HIPAA", "HITRUST", "FedRAMP_MODERATE", "FedRAMP_HIGH"], var.compliance_regime)
    error_message = "compliance_regime must be one of: HIPAA, HITRUST, FedRAMP_MODERATE, FedRAMP_HIGH."
  }
}

variable "assured_workloads_location" {
  description = "The GCP region or multi-region where Assured Workloads resources must reside (data residency constraint)."
  type        = string
  default     = "us-central1"
}

variable "enable_cmek" {
  description = "Whether to enforce CMEK (Customer-Managed Encryption Keys) org policies."
  type        = bool
  default     = true
}

variable "enable_access_transparency" {
  description = "Whether to enforce Access Transparency policies required for HIPAA."
  type        = bool
  default     = true
}
